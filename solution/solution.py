...
class RAGASEvaluator:
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        return min(
            1.0,
            max(0.0, len(expected_tokens & context_tokens) / len(expected_tokens)),
        )
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            relevant_at_rank.append(coverage >= relevance_threshold)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = f"""You are an impartial response evaluator. Score the answer against each rubric criterion.
Return a JSON object whose keys are the criterion names and whose values are scores from 0.0 to 1.0. Do not reward length by itself.
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for result in scores_batch:
            ...
            if isinstance(scores, dict):
                ...
                first = scores.get("first_response", result.get("first_response"))
                second = scores.get("second_response", result.get("second_response"))
                if (
        ...
        first_wins = sum(first > second for first, second in position_pairs)
        positional_bias = bool(position_pairs) and first_wins / len(position_pairs) >= 0.8
        return {
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = (
                ...
                else (suggestions[-1] if suggestions else "Investigate this failure.")
            )
            cells = (
                str(pair_id),
    ...
    def generate_improvement_suggestions(
        ...
        suggestions_by_type = {
            "hallucination": (
                "Add claim-to-context grounding checks and require the generator "
                "to state when evidence is insufficient."
            ),
            "irrelevant": (
                "Improve intent routing and add prompt examples that explicitly "
                "answer the customer's primary question."
            ),
            "incomplete": (
                "Increase evidence coverage with better retrieval/chunking and ask "
                "the generator to include all conditions and exceptions."
            ),
            "off_topic": (
                "Add intent validation before generation and regression cases for "
                "ambiguous or multi-part questions."
            ),
            "refusal": (
                "Review guardrail rules and add examples that distinguish safe "
                "OrbitTech requests from requests that require refusal."
            ),
            "unknown": (
                "Inspect answer and retrieval traces, then label the failure before "
                "choosing a targeted improvement."
            ),
        }
        ordered_types = sorted(
            categories, key=lambda category: (-categories[category], category)
        )
        suggestions = [
            suggestions_by_type.get(category, suggestions_by_type["unknown"])
            for category in ordered_types
        ]
        general_suggestions = [
            "Review the lowest-scoring answer and retrieval traces before changing "
            "the system so the fix targets the observed cause.",
            "Add each confirmed failure pattern to the golden dataset as a regression "
            "case and rerun the benchmark after the fix.",
            "Track the affected metric against the current baseline to verify that "
            "the improvement does not create a regression elsewhere.",
        ]
